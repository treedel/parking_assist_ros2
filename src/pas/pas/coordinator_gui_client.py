...
class ROS2Interface(Node):
    def __init__(self):
        ...
        self.speed = 0.0
        self.create_subscription(Twist, '/prius_hybrid/cmd_vel', self.cmd_vel_callback, 10)

    def cmd_vel_callback(self, msg):
        self.speed = (msg.linear.x ** 2 + msg.linear.y ** 2 + msg.linear.z ** 2) ** 0.5

class Dashboard(QWidget):
    def __init__(self, robot, ros_node, number):
        super().__init__()
        self.robot = robot
    ...
    def init_ui(self):
        ...
        package_path = get_package_share_directory("pas")
        icon_path = os.path.join(package_path, "config", "car.png")
        ...
        self.speed_label.setFont(font_large)
    ...
    def start_guidance_system(self):
        ...
        self.robot.goToEulerPose(pos)
        self.status_label.setText("Status: Guidance system online")

    def update_dashboard(self):
        now = datetime.datetime.now().strftime("%H:%M:%S")
        self.time_label.setText(now)
        self.speed_label.setText(f"{self.ros_node.speed:.1f} m/s")

def main():
    rclpy.init()
    number = input("Enter car registration number: ")
    robot = RobotControl()
    ros_node = ROS2Interface()
    app = QApplication(sys.argv)
    dashboard = Dashboard(robot, ros_node, number)
    dashboard.show()
